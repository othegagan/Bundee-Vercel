...
const DrawerOverlay = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Overlay>>(
    ({ className, ...props }, ref) => (
        <DrawerPrimitive.Overlay ref={ref} className={cn('fixed inset-0 z-50 bg-black/50 backdrop-blur-[2px]', className)} {...props} />
    ),
);
...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, DrawerContentProps>(
    ({ className, children, onDragClose, ...props }, ref) => {
        ...
        const handleDragStart = (event: React.TouchEvent | React.MouseEvent) => {
            setIsDragging(true);
            setStartY(event.nativeEvent instanceof TouchEvent ? event.nativeEvent.touches[0].clientY : event.nativeEvent.clientY);
        };

        const handleDragMove = (event: React.TouchEvent | React.MouseEvent) => {
            if (!isDragging) return;
            const currentY = event.nativeEvent instanceof TouchEvent ? event.nativeEvent.touches[0].clientY : event.nativeEvent.clientY;
            if (currentY - startY > 100) {
                // Adjust the threshold as needed
                setIsDragging(false);
                if (onDragClose) onDragClose();
            }
        };
        ...
        return (
            <DrawerPortal>
                ...
                <DrawerPrimitive.Content
                    ...
                    <div className=' max-h-[60vh] overflow-y-auto mb-4'>{children}</div>
                </DrawerPrimitive.Content>
            </DrawerPortal>
        );
    },
