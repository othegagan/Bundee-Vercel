...
const InputComponent = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => (
    <Input type='number' className={cn('rounded-e-lg rounded-s-none', className)} {...props} ref={ref} />
));
        ...
        <Popover>
            ...
            <PopoverContent className='w-[300px] p-0'>
                <ScrollArea className='border-1 flex max-h-60 w-[300px] select-none flex-col rounded-lg p-1'>
                    {options.map(option => (
                        <div
                            className='flex items-center gap-2 rounded-md px-2 py-1 hover:bg-foreground/10'
                            key={option.value}
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
    ...
    return <span className='flex h-4 w-6 overflow-hidden rounded-sm bg-foreground/20'>{Flag && <Flag title={countryName} />}</span>;
};
