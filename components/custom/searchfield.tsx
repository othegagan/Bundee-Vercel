import { Button, ButtonProps, Input, InputProps, SearchFieldProps, SearchField as _SearchField } from 'react-aria-components';

import { cn } from '@/lib/utils';

function SearchFieldInput({ className, ...props }: InputProps) {
    return (
        <Input className={cn('min-w-0  flex-1 bg-background px-2 py-1.5 outline outline-0 [&::-webkit-search-cancel-button]:hidden', className)} {...props} />
    );
}
