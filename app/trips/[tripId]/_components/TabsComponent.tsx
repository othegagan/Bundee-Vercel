...
export default function TabsComponent({ tripId }: Readonly<{ tripId?: string }>) {
        ...
        <div
            ...
            <Link
                ...
                className={cn(
                    ...
                    pathname === `/trips/${tripId}/details` ? 'bg-primary text-white shadow' : 'bg-neutral-100 text-muted-foreground',
                )}>
                Trip Details
            </Link>

            <Link
                href={`/trips/${tripId}/conversation`}
                type='button'
                role='tab'
                className={cn(
                    'inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
                    pathname === `/trips/${tripId}/conversation` ? 'bg-primary text-white shadow' : 'bg-neutral-100 text-muted-foreground',
                )}>
                Conversation
            </Link>
        </div>
